...
describe("Population", function (): void {
    ...
    describe("Subtree", function (): void {
      for (const chance of [0, 0.1, 1]) {
        it(`Chance ${chance}`, function (): void {
          ...
          const mutate = createSubtreePopulationMutator(
            "TEST",
            chance,
            (): Statement<Inputs> => c5
          );
  ...
  describe("Competition", function (): void {
    it("Evaluate One", function (): void {
      ...
      snapshot(pc.evaluateOne(c1));
      snapshot(pc.evaluateOne(c2));
      snapshot(pc.evaluateOne(c3));
      snapshot(pc.evaluateOne(o1));
      snapshot(pc.evaluateOne(o2));
      snapshot(pc.evaluateOne(o3));
      snapshot(pc.evaluateOne(o4));
