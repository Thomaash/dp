...
(async () => {
  ...
  for (const course of courses["trafIT"]["courses"][0]["course"]) {
    ...
    // Remove duplicates.
    const visitedItineraryNames = new Set();
    course.itinerary = course.itinerary
      .filter((itinerary) => {
        if (visitedItineraryNames.has(itinerary.$.name)) {
          return false;
        } else {
          visitedItineraryNames.add(itinerary.$.name);
          return true;
        }
      })
      .map((itinerary, i) => {
        // Fix potential holes in priorites.
        itinerary.$.priority = 1 + i;
        return itinerary;
      });
  }
