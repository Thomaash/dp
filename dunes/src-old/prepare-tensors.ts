...
export type PreparedTensor = [Tensor2D, Tensor1D];
...
export async function prepareSample(
  tf: TFJS,
  path: string
): Promise<{ xs: Tensor2D; ys: Tensor1D }> {
  ...
  try {
    ...
    const xShape: [number, number] = tensorData.shape;
    const dType: DataType = tensorData.dType;

    const xs = tf.tensor2d(tensorData.x, xShape, dType);
    const ys = tf.tensor1d(tensorData.y, "float32");
