...
const { OTMessages, OTTimetableStatistics } = require("../dist/otapi");

const outputsPath = process.argv[2];
const outputsBasename = path.basename(outputsPath);
const outputsDirname = path.dirname(outputsPath);
...
function getMessages(runDirname) {
  const messagesPath = path.resolve(outputsPath, runDirname, "OT_Messages.txt");
...
function fetchDelX(type, run, runDirname) {
  const delXPath = path.resolve(outputsPath, runDirname, "OT_Delay.del" + type);
...
function getStuck(results) {
  ...
  return getFailedRunSummary(
    runs,
    (run) => run.trainIDs,
    [...runs].filter((run) => run.stuck > 0),
    (run) => run.stuckCourseIDs
  );
}

function getXX(results) {
  const runs = [...new Set(results.map((result) => result.run))];
  return getFailedRunSummary(
    runs,
    (run) => run.trainIDs,
    [...runs].filter((run) => run.xx > 0),
    (run) => run.xxCourseIDs
  );
}

function getFailedRunSummary(allRuns, reduceTotal, failedRuns, reduceFailed) {
  const modules = new Set([...failedRuns.map((run) => run.module)]);

  return {
    length: failedRuns.length,
    shortText: `${nOutOf(failedRuns.length, allRuns.length)}; ${failedRuns
      ...
      .join(", ")}`,
    detailedText: `${nOutOf(failedRuns.length, allRuns.length)}\n${failedRuns
      .sort((a, b) => a.runNumber - b.runNumber)
      .map(
        (run) =>
...
function loadResults() {
  ...
  const allDirnames = fs
    .readdirSync(outputsPath, { withFileTypes: true })
  ...
  for (const suffix of suffixes) {
    ...
    runDir: for (const runDirname of runDirnames) {
      for (const requiredFile of REQUIRED_FILES) {
        const requiredFilePath = path.resolve(
          outputsPath,
          runDirname,
          requiredFile
        );
        if (!fs.existsSync(requiredFilePath)) {
          console.warn(
            `Warning: ${requiredFilePath} doesn't exist, skipping ${path.resolve(
              outputsPath,
              runDirname
            )}.`
          );
      ...
      const otTimettableStatistics = new OTTimetableStatistics(
        fs.readFileSync(
          path.resolve(outputsPath, runDirname, "OT_TimetableStatistics.txt"),
          "utf-8"
        )
      );
...
(async () => {
  const { results: allResults, runs: allRuns } = loadResults();
  console.info();
  ...
  if (allResults.length >= 2) {
    ...
    const xx = getXX(allResults).detailedText;
  ...
  for (const type of ["perCategoryDiffs", "perCourseDiffs"]) {
    const diffCategories = [
      ...new Set(allRuns.flatMap((run) => [...run.delays[type].keys()])),
    ].sort();

    const modulesAndDiffCategories = modules.flatMap((module) =>
      diffCategories.map((diffCategory) => ({ diffCategory, module }))
    );
    ...
    const keys = [
      ...
      ...modules.flatMap((module) => [
        "",
        ...diffCategories
          .filter((diffCategory) => diffCategory !== "total")
          .map((diffCategory) => csvHeader(diffCategory, module)),
    ...
    const rows = runNumbers
      ...
      .filter(
        (runNumber) =>
          new Set(
            allRuns
              .filter((run) => run.runNumber === runNumber)
              .map((run) => run.module)
          ).size === modules.length
      )
      // Turn runs into CSV rows.
      .map((runNumber) => {
        const runs = allRuns.filter((run) => run.runNumber === runNumber);

        return {
          run: runNumber,
          ...modulesAndDiffCategories.reduce(
            (acc, { diffCategory, module }) => {
              const run = runs.find((run) => run.module === module);

              // In minutes for now.
              // TODO: Evaluate ideal units/format.
              acc[csvHeader(diffCategory, module)] =
                run.delays[type].get(diffCategory) / 60;
    ...
    fs.writeFileSync(
      path.resolve(outputsDirname, `${outputsBasename}.delays.${type}.csv`),
      csv
    );
  }
})();
