  ...
  for (const suffix of suffixes) {
    ...
    for (const runDirname of runDirnames) {
      for (const requiredFile of REQUIRED_FILES) {
        const requiredFilePath = path.join(root, runDirname, requiredFile);
        if (!fs.existsSync(requiredFilePath)) {
          console.warn(
            `Warning: ${requiredFilePath} doesn't exist, skipping ${path.join(
              root,
              runDirname
            )}.`
          );
      ...
      const otTimettableStatistics = new OTTimetableStatistics(
        fs.readFileSync(
          path.join(root, runDirname, "OT_TimetableStatistics.txt"),
          "utf-8"
        )
      );
