...
(async () => {
  ...
  const firstRunDate = results
    ...
    .reduce((acc, date) => (acc < date ? acc : date), Number.POSITIVE_INFINITY);
  const lastRunDate = results
    .map((result) => result.run.date)
    .reduce((acc, date) => (acc > date ? acc : date), Number.NEGATIVE_INFINITY);
  ...
  const avgRunDuration = formatDistanceStrict(
    firstRunDate,
    new Date(
      +firstRunDate + Math.round((lastRunDate - firstRunDate) / numberOfRuns)
    )
  );
