...
function singleModuleToString(module, type, allModuleResults) {
  ...
  if (stuck.length > 0) {
    moduleLines.push(`  Stuck runs: ${stuck.shortText}`);
  }
...
(async () => {
  ...
  for (const module of resultsOfModules) {
    ...
    for (const type of types) {
      const moduleTypeResults = allResults
        .filter(
          (result) => result.run.module === module && result.type === type
        )
        .sort((a, b) => a.runNumber - b.runNumber);

      const keys = [
        "run",
        ...trainTypes.map((trainType) => csvHeader(trainType)),
        csvHeader("total"),
      ];
      const rows = runNumbers.map((runNumber) => {
        const runResults = moduleTypeResults.filter(
          (result) => result.run.runNumber === runNumber
        );

        return {
          run: runNumber,
          [csvHeader("total")]: computeAverageDelay(runResults),
          ...trainTypes.reduce((acc, trainType) => {
            const trainTypeResults = runResults.filter((result) =>
              result.trainID.startsWith(trainType + " ")
            );

            acc[csvHeader(trainType)] = computeAverageDelay(trainTypeResults);
      ...
      fs.writeFileSync(path.resolve(root, `${module}.${type}.csv`), csv);
    }
  }
})();
